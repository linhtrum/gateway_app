...
export function Sidebar({ currentRoute }) {
  const links = [
    ...
    },
    {
      path: "/management",
      label: "Management",
      icon: html`<${Icons.ResetIcon} className="w-5 h-5" />`,
    },
  ];
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      >
        ...
        <div class="flex items-center space-x-2">
          <button
            onClick=${onEdit}
            class="w-8 h-8 flex items-center justify-center rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-all"
          ...
          <button
            onClick=${onDelete}
            class="w-8 h-8 flex items-center justify-center rounded text-gray-400 hover:text-white hover:bg-red-500 transition-all"
            title="Delete card"
          >
...
export function CardModal({ isOpen, onClose, onSubmit, card = null, isEditing = false, devices = [] }) {
    ...
    if (isOpen) {
      if (isEditing && card) {
        ...
        if (deviceIndex !== -1) {
          ...
          const tempNodeIndex = device.ns.findIndex(node => node.n === card.tn.n);
          const humidNodeIndex = device.ns.findIndex(node => node.n === card.hn.n);
  ...
  const handleSubmit = (e) => {
    ...
    if (!device.ns[parseInt(formData.ti)] || !device.ns[parseInt(formData.hi)]) {
